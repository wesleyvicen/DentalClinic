...
@Service public class AgendaServiceImpl implements AgendaService {
    ...
    @Autowired private PrestadorServiceImpl prestadorService;

    @Transactional(readOnly = true) public List<Agenda> findAll() {
    ...
    @Transactional public List<Agenda> getAgendasWithLogin(String login) {
        List<Agenda> list = agendaRepository.getAgendasWithLogin(login);
        return list;
    }

    @Transactional public List<Agenda> getAgendasWithDateBetween(String login, LocalDate dataInicio,
        LocalDate dataFim) {
    ...
    public List<Agenda> getAgendasWithDateBetweenWithPrestador(String login, LocalDate dataInicio,
        ...
        List<Agenda> list =
            agendaRepository.getAgendasWithDateBetweenWithPrestador(login, dataInicio, dataFim,
                prestadorId);
        return list;
    }
    ...
    @Transactional public Agenda insert(AgendaDTO obj) {
    ...
    private void updateData(Agenda newObj, Agenda obj) {
        ...
        newObj.setPagamento(
            obj.getPagamento() == null ? newObj.getPagamento() : obj.getPagamento());
        newObj.setDetalhes(obj.getDetalhes() == null ? newObj.getDetalhes() : obj.getDetalhes());
    }

    public Agenda fromDTO(AgendaDTO objDto) {
        Agenda agenda =
            new Agenda(objDto.getId(), objDto.getTitle(), objDto.getStart(), objDto.getEnd(),
                objDto.getAllDay(), objDto.getValor(), objDto.getStatus(), objDto.getPagamento(),
                objDto.getDetalhes());
    ...
    @Transactional public List<Agenda> getAgendasWithStatus(Integer status) {
        List<Agenda> list = agendaRepository.getAgendasWithStatus(status);
        return list;
    }

    @Transactional public List<Agenda> getAgendasWithLoginAndStatus(String login, Integer status) {
