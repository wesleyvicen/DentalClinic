...
public interface AgendaRepository extends JpaRepository<Agenda, Long> {
	...
	@Query(value = "SELECT agenda FROM Agenda agenda LEFT JOIN FETCH agenda.paciente paciente WHERE agenda.paciente.id = :paciente_id ORDER BY agenda.start DESC")
	...
	@Query("Select agenda from Agenda agenda where agenda.usuario.login = :loginUsuario and (:prestadorId = 'all' or agenda.prestador.id = :prestadorId) and agenda.allDay between :dataInicio and :dataFim")
	List<Agenda> getAgendasWithDateBetweenWithPrestador(@Param("loginUsuario") String loginUsuario,
			@Param("dataInicio") LocalDate dataInicio, @Param("dataFim") LocalDate dataFim,
			@Param("prestadorId") String prestadorId);
